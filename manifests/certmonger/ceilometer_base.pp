...
# under the License.
#
# == Class: tripleo::certmonger::ceilometer_base
#
...
#
# [*principal*]
#   (Optional) The haproxy service principal that is set for ceilometer_base in kerberos.
#   Defaults to undef
#
class tripleo::certmonger::ceilometer_base (
  $hostname,
  ...
  include ::certmonger
  include ::ceilometer_base::params

  $postsave_cmd_real = pick($postsave_cmd, "systemctl restart ${::ceilometer_base::params::service_name}")
  certmonger_certificate { 'ceilometer_base' :
    ensure       => 'present',
  ...
  file { $service_certificate :
    require => Certmonger_certificate['ceilometer_base'],
  }
  file { $service_key :
    require => Certmonger_certificate['ceilometer_base'],
  }

  File[$service_certificate] ~> Service<| title == $::ceilometer_base::params::service_name |>
  File[$service_key] ~> Service<| title == $::ceilometer_base::params::service_name |>
}
